...
class Validator
{
     ...
     *
     * @see class `Tivins\Schema\Schema_Java`, class `Tivins\Schema\Schema_Common`.
     */
    static public function validate_schema(Token $start_token, array $schema)
    {
         ...
         */
        $last_key = \Tivins\Abstract\Util::get_last_key($schema);
        ...
        foreach ($schema as $key => $iter)
        {
            ...
            */
            ...
            //     ."), data(".\Tivins\Abstract\Util::inline_str($current->data)." | ".gettype($current->data).")") ;
     ...
     * @return void
     */
    static public function apply_object_model(array &$schema, \Tivins\ObjectModel\OObject $object)
    {
        foreach ($schema as $item) {
            if (isset($item['token']) && $item['token']) {
