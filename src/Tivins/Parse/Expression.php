...
class Expression
{
    ...
    static private function reduce_expressions(Token $root)
    {
        do
        {
            ...
            // \Msg::funcline(__function__, "Remains expression ? ".\Tivins\Abstract\Util::yesno($remains));
        } while ($remains) ;
    }
    ...
    static private function parse_num_ope(Token $root)
    {
        ...
        $order = \Tivins\Schema\Schema_Common::get_operators_precedence();
    ...
    {
        $matches = [];
        $schema = \Tivins\ObjectModel\Lang::get_schema('Common','number_operation') ;
        $level = 0 ;
        $current = $root ;
        while ($current) {
    ...
    static private function parse_unary(Token $root)
    {
        ...
        while ($current) {
            ...
            if ($current->check(Token::TYPE_PUNCT) && in_array($current->data, \Tivins\Abstract\Punct::get_unary()))
            {
                $previous = $current->get_next_non_code(\Tivins\Abstract\Node::DIR_LEFT);
                $next = $current->get_next_non_code(\Tivins\Abstract\Node::DIR_RIGHT);
            ...
            $schema1 = [
                'good'  => ['qty' => 1, 'optional' => 0, 'type' => Token::TYPE_PUNCT, 'data' => \Tivins\Abstract\Punct::get_operators()],
                'unary' => ['qty' => 1, 'optional' => 0, 'type' => Token::TYPE_PUNCT, 'data' => \Tivins\Abstract\Punct::get_unary()],
