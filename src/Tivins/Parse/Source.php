...
namespace Tivins\Parse;

use Tivins\Abstract\Msg;
...
class Source
{
     ...
     */
    private ?Token $last_added_token = null ;
     ...
     * content.
     *
     * @return  Source|false Returns a valid `Source` object or `false` if the file doesn't
     *          exists or if it's not readable.
     *
     * @todo    To transform `false` to `null` and use nullable
     *          return type: `:?Source`.
     */
    static public function fromFile(string $filename): bool|Source
    {
    ...
    public function parse(Tivins\Abstract\Parser $parser)
    {
        $this->parser = $parser ;
    ...
    */
    ...
    public function treefy():bool
    {
        ...
        if ($level > 0)
        {
            \Tivins\Parse\Compiler_Error::emit(\Tivins\Parse\Compiler_Error::structure_error, $tkparent);
        }
        return $level == 0 ;
    }
    ...
    public function tokenize(): bool
    {
        ...
        while ($wk->is_inside()) {
            $char = $wk->get_char();
            // Msg::debug("char[$wk->idx]\t'".\Tivins\Abstract\Util::inline_str($char)."'\t".ord($char)) ;
            ...
            if (Punct::is_letter($char)) {
                ...
                if (strpos($token->data, '..')) {
                    Compiler_Error::emit(Compiler_Error::syntax, $token, '`..` detected.');
                }
                if (trim($token->data, '.') != $token->data) {
                    Compiler_Error::emit(Compiler_Error::syntax, $token, '`.` at the begin or the end of name.');
                }
                continue;
            }
    ...
    private function parse_until($end_seq, &$found = false): string
    {
        $wk = $this->walker ;
    ...
    private function parse_group($func): string
    {
        $wk = $this->walker ;
    ...
    private function parse_string($start = '"'): string
    {
        $wk = $this->walker;
     ...
     * @todo Set static.
     */
    public function trim_tokens(): void
    {
        $level = 0 ;
        $current = $this->root ;
        while ($current) {
